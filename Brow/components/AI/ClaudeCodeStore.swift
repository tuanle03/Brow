...
final class ClaudeCodeStore: ObservableObject {
    ...
    func handlePermissionRequest(_ payload: PermissionRequestPayload, rawJSON: String) async -> String {
        ...
        let decision = await withCheckedContinuation { (cont: CheckedContinuation<ApprovalDecision, Never>) in
            ...
            // the session. The previous order ran updateSession first,
            // which published a session whose `lastTool` projected through
            // the registry as a Monitor row with "<tool>…" status. The
            // notch opened on that Monitor frame and only switched into
            // the Approve section once the second @Published — pending —
            // landed, which read to the user as a "notification flash"
            // before approve. Flipping the order keeps the registry in
            // `.approve` mode from the very first projection (orphan
            // pending is enough), and `updateSession` then rewrites the
            // orphan into the proper session-backed task with no visual
            // transition.
